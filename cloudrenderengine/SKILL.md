...
单例控制器 (通过 engine.xxx 访问):
├── l3ModelControl (L3精模控制)
├── explodeControl (模型爆炸/拆卸)
├── skeletonAnimControl (骨骼动画)
